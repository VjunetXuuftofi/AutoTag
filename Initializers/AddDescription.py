...
This reads in a spreadsheet of loans and creates a new spreadsheet that includes the loan description, whether the loan
is only to women, and whether the borrower has borrowed from Kiva before.
"""
...
for i in range(len(everyloan)):
    ...
    for a in tqdm(range(100)):
        try:
            loanstowrite[i*100+a]["Description"] = everyloan[i][a]["description"]["texts"]["en"]
            valid = True
            ...
            repeatborrower = 0
            soup = BeautifulSoup(requests.get("http://www.kiva.org/lend/" + str(everyloan[i][a]["id"])).text, "html.parser")
...
towrite = csv.DictWriter(open("/Users/thomaswoodside/PycharmProjects/AutoTag/DataFiles/loans_assigned_for_tagging_with_descriptions_new.csv",
                              "w+"),
                         fieldnames=["Loan ID","Name","Raw Link","Loan Link For Excel","Popularity","Loan Amount",
                                     "Funded Amount","Amount Needed","Percent Funded","Lars Ratio",
                                     "Time Left (Seconds)","Funding Rate Per Hour","Posted Date (UTC)",
                                     "Planned Expiration Date (UTC)","Disbursed Date (UTC)","Posted Date (US/Pacific)",
                                     "Planned Expiration Date (US/Pacific)","Disbursal Date (US/Pacific)","Time Left",
                                     "Partner Name","Partner Link","Partner Delinquency Rate","Partner Default Rate",
                                     "Partner Total Amount Raised","Partner No. of Loans Posted",
                                     "Partner Loans At Risk Rate","Partner Currency Exchange Loss Rate",
                                     "Partner Portfolio Yield","Partner Rating","Partner Secular Rating",
                                     "Partner Social Rating","Partner Religious Affiliation","Country","Sector",
                                     "Activity","Use","Tags","Repayment Interval","Repayment Term","Translator Byline",
                                     "Themes","Ray Number","Ray Total","Ray Array","Loan Tagger","Assigned On (UTC)",
